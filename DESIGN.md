...
- Auto-fit grids for repeating content: talks at `minmax(300px, 1fr)`, project paragraphs at `minmax(min(100%, 340px), 1fr)`, and ordinary multi-image project figures at `minmax(min(100%, 360px), 1fr)`. ASKO groups its copy and screenshots into separate ASKO Servering and Fiks sections; all three dense interface screenshots stay full-width and in source order. A lone project figure uses a centered 12-column composition with 12px gutters and spans ten columns. Rewindify uses a 12-column grid with a 32px gutter to keep its 9-column desktop screenshot distinct from its 3-column mobile screenshot; below 900px the desktop view spans the row and the phone stays centered at up to 360px.
