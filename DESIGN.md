...
Density is generous between regions and deliberately uneven inside the projects region: 96px between major sections and up to 128px around the projects region, but 80px inside the lead project row against 24px inside a corroborating one. That gap between the two row rhythms is the hierarchy — the lead row is a block of type and evidence, the other two are lines in a ledger — and it is why the region reads in one pass. Hairlines rather than gaps where two things need separating, and a hairline only where two rows meet, never closing a list at its top and bottom. The region carries exactly two rules for its three rows; everything else there is type on a ground. The confirmed rejections are the dark developer-portfolio default (there is no dark mode and no dark ground except the deliberate ink footer), cards-as-structure (there is no card at rest anywhere in the system), and faded text as an emphasis device (see The Never-Fade Rule).
...
- A corroborating row is a line, not a panel: one sentence of claim, one line of meta, and the space after it reads as the end of a line rather than as an unfinished column
...
Depth is instead carried by three devices: a change of ground (paper → deep paper → field → ink), a hairline (1px `rule` for a division, 1px `rule-strong` for an edge, 2px `ink` for a structural break above a section-defining element), and a 2px `translateY` lift on the primary buttons that returns to 0 on `:active`. The projects region adds a fourth: a row flips its ground from field to paper on hover and focus, which is the whole of its depth vocabulary — nothing lifts, nothing scales, nothing fades. That one is large enough that its timing is part of the device rather than a detail of it; see The Ground-Scale Motion Rule.
...
- **The entry.** Title at `--step-2`, blurb at 58ch, one meta line, then the affordance, stacked in a single column at 8px with the affordance 8px under the meta. Everything the reader needs is read downward in one place rather than tracked across four columns that each held one fact.
...
- **The meta line.** Period in mono, a decorative `·`, the role, another `·`, and the technologies — `2021–2023 · Lead frontendutvikler · React Next.js Tailwind CSS`, one line, read as one sentence. All three are measured values a buyer needs before deciding whether the click is worth it: role answers solo-or-embedded, the year turns a stack list from a possibly-stale claim into a dated one. The bare year keeps its `.visually-hidden` "Periode" label so a screen reader hears a fact rather than a loose number, and both separators are `aria-hidden` because the parts are already separate elements.
- **Why only the lead row breaks its stack.** `flex-basis: 100%` on the stack is the lead row's rule alone: a Norwegian six-technology list does not fit beside a dated role at any width this site has, and a `·` left hanging at the end of a wrapped line reads as a mistake rather than as a divider. A three-technology list does fit, so a corroborating row keeps everything on one line — and the second `·` only renders on those rows, because the lead row's break would leave it dangling.
...
- **The paper flip.** On `:hover` and `:focus-visible` the row sets `background: var(--paper)`, `color: var(--ink)`, and `--focus: var(--ink)` in one rule, and swaps two inherited variables — `--row-secondary` (field-mist → soft ink) and `--row-affordance` (field-mist → signal) — so the blurb, the meta line, the caption and the underline follow the ground rather than being restated. This is the region's entire depth vocabulary: nothing lifts, nothing scales, nothing fades.
- **The flip's clocks.** Three of them, declared as two variables on the row and inherited by everything inside it, because this is the largest state change on the site and the timing is the whole difference between a sheet arriving and the region losing its ground. The **ground** cross-fades on `--ease-ground` over `--flip`: 320ms arriving, 240ms leaving. The **ink** does not travel with it — it is reprinted in one step at `--type-swap`, 35% of the way in and 65% of the way out (see The Reprinted-Ink Rule). The **cues** keep their own fast clocks — 200ms on the underline's weight, 220ms on the arrow — so the pointer is answered immediately whatever the ground is doing. A hover that was never meant, the pointer crossing a row on its way somewhere else, resolves to nothing but a breath of ground: it does not reach the ink switch, and both ends of the curve are slow.
- **Why 320ms rather than 200.** At 200ms of `ease` — a curve that leaves at speed — 1160 × 485px of band moving 66 points of lightness reads as a flash, and it fires on every pointer that merely crosses the row. This is the one element on the site over the area threshold in The Ground-Scale Motion Rule, and it is the reason that rule exists.
...
- **Why the orange arrives late and leaves first.** Signal is illegal on the field in any form, and a colour held in a plain custom property is a token swap rather than a value: it snapped orange at t=0, over a row still three-quarters green. `--row-affordance` is therefore registered with `@property` as a `<color>`, which is what makes it interpolable, and it rides the ground's clock in — the orange resolves as the paper does, and at no frame is there signal on a green ground. Out, it has no duration at all: it goes the instant the pointer does rather than fading across a ground turning green underneath it. This is the only registered custom property in the system, and the reason to register one is exactly this — a variable that has to be timed, not just swapped.
...
- **Reduced motion:** The global reduced-motion block clamps the transitions to 0.01ms **and the delays to 0**; the flip still happens, instantly and in one piece, because it is the affordance and not an embellishment. Without the delay clamp the ground would land at once while the ink waited out `--type-swap` — paper text on a paper sheet, which is why that clamp is not optional for any state built as a choreographed change.
...
Three easings and a short duration ladder. `ease` for pure colour changes (140–200ms); `cubic-bezier(0.16, 1, 0.3, 1)` for anything that moves or grows (180ms button lift, 200ms caret, 220ms underline growth and icon nudge); and `--ease-ground`, `cubic-bezier(0.45, 0, 0.55, 1)`, for a whole ground cross-fading — a sine curve, slow at both ends, used only by the project row's flip at 320ms in and 240ms out. `prefers-reduced-motion: reduce` clamps all animation and transition durations to 0.01ms **and all transition delays to 0**, and turns off `scroll-behavior: smooth`. There is no JavaScript-driven motion anywhere on the site.

**The Ground-Scale Motion Rule.** The third easing is a token rather than a literal because it carries a size threshold: past roughly a section's worth of area, a colour change stops being a state and becomes a page event, and a curve that leaves at speed reads as a flash. The project row is the only element on the site above that threshold — 1160 × 485px of full-bleed band moving 66 points of lightness. Anything that big cross-fades on `--ease-ground` and is timed in the 240–320ms band; everything smaller keeps `ease` at 140–200ms. Two consequences bind to it: leaving is quicker than arriving, so a pointer run down a list never leaves two rows holding half a sheet each, and a slow ground never delays the small geometric cues — the underline weight and the icon nudge keep their own fast clocks, so the pointer is always answered at 200ms whatever the ground is doing.

**The Reprinted-Ink Rule.** Type does not cross-fade with a ground it inverts against. Paper ink and a half-arrived paper sheet pass through the same lightness, so a lockstep fade drops a whole column of copy to 1.3:1 on its way to ink — the Never-Fade Rule's defect, arrived at by transition rather than by declaration. The ink switches instead, in one step, at the point in the fade tuned for the primary ink: 35% of the way in while the ground is still dark, 65% of the way out once the ground is dark again, measured at no worse than 3.5:1 on either side of the switch. Secondary text cannot be given the same guarantee — soft ink and field mist sit on either side of the ground's own path, so one of them always collides with it, and the swap point is chosen to make that window as short and as shallow as it can be rather than to eliminate it. Implemented as a zero-duration transition with a delay (`color 0s var(--type-swap)`), which is also why the reduced-motion block has to clamp delays as well as durations. A pointer merely crossing a row never reaches the switch at all — the ground breathes and the type never moves.
...
- **Do** cross-fade a section-sized ground on `--ease-ground` in the 240–320ms band, out quicker than in, and register the variable as a `<color>` with `@property` if a colour it carries has to be timed rather than swapped.
- **Do** switch inverting type in one step, part-way through the ground it is inverting against, rather than fading it alongside — and clamp `transition-delay` alongside `transition-duration` in any reduced-motion block that has to hold such a state together.
...
- **Don't** put signal orange on the green field in any form — including for the length of a transition. In the projects region it becomes available only as a row's own ground arrives at paper, and it leaves before that ground does.
- **Don't** cross-fade paper text and ink text through each other over a ground that is inverting underneath them. The two pass through the same lightness and the copy drops to 1.3:1 in the middle of the fade — a fade this system refuses, arrived at by transition instead of by declaration.
- **Don't** put a large ground change and a small pointer cue on the same clock. The ground is allowed to be slow; the underline and the arrow that answer the pointer are not.
